...
describe('Profile Routes', function() {
  ...
  afterEach(done => {
    Promise.all([User.remove({}), Profile.remove({}), Location.remove({})])
      .then(() => done())
      .catch(done);
  });
  ...
  describe('POST: /api/profile', function() {
    ...
    describe('without a token', () => {
      it('should return a 401 error', done => {
        request.post(`${url}/api/profile`).send(testProfile).end(err => {
  ...
  describe('GET: /api/profile/:id', function() {
    ...
    describe('with a valid request', () => {
      it('should return a profile', done => {
        request
          .get(`${url}/api/profile/${this.tempProfile._id}`)
          .set({
            Authorization: `Bearer ${this.tempToken}`,
          })
          .end((err, res) => {
    ...
    describe('without a token', () => {
      it('should return a 401 error', done => {
        request.get(`${url}/api/profile/${this.tempProfile._id}`).end(err => {
          expect(err.status).to.equal(401);
          done();
        });
      });
  ...
  describe('GET: /api/profile/user', function() {
    ...
    describe('without a token', () => {
      it('should return a 401 error', done => {
        request.get(`${url}/api/profile/user`).end(err => {
          expect(err.status).to.equal(401);
          done();
        });
      });
  ...
  describe('DELETE: /api/profile', function() {
    ...
    afterEach(done => {
      ...
      Message.remove({}).then(() => done()).catch(done);
    });
    ...
    describe('without a token', () => {
      it('should return a 401 error', done => {
        request.delete(`${url}/api/profile`).end(err => {
          expect(err.status).to.equal(401);
          done();
        });
      });
  ...
  describe('GET: /api/profile', function() {
    ...
    describe('without a token', () => {
      it('should return a 401 error', done => {
        request.get(`${url}/api/profile`).end(err => {
          expect(err.status).to.equal(401);
          done();
        });
      });
